...
contract MultiSigEarningsAndFees is AccessControl, ReentrancyGuard {
    ...
    bytes32 public constant ADMIN4_ROLE = keccak256("ADMIN4");
    bytes32 public constant ADMIN5_ROLE = keccak256("ADMIN5");
    ...
    function isOwner(address addr) private view returns(bool) {
        return (hasRole(ADMIN1_ROLE, addr) || hasRole(ADMIN2_ROLE, addr) || hasRole(ADMIN3_ROLE, addr) || hasRole(ADMIN4_ROLE, addr) || hasRole(ADMIN5_ROLE, addr));
    }
    ...
    constructor(address[] memory _owners, uint _confirmations) {
        ...
        _setupRole(ADMIN4_ROLE, msg.sender);
        _setupRole(ADMIN5_ROLE, msg.sender);
